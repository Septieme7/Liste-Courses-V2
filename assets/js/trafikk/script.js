/* =============================================================
  COURSES MALIN · script.js (version enrichie et corrigée)
============================================================= */
...
// Pour le glisser-déposer des listes
let listsSortable = null;

// Pour conserver l'ordre original des listes
let originalListsOrder = [];

// Pour la recherche de listes
let listSearchActive = false;
let lastListQuery = '';

// Pour le sélecteur d'emoji
...
async function init() {
  ...
  originalListsOrder = lists.map(l => l.id);
}
...
function saveCategory() {
  ...
  renderHome();
  // Fermeture forcée
  forceCloseSheet('shCategoryEdit');
}
...
function goTo(tab) {
  ...
  if (fabSearch) fabSearch.style.display = onLists ? 'flex' : 'none';
...
function renderLists() {
  ...
  if (grid) {
    grid.innerHTML = lists.map(list => {
      ...
      return `
        <div class="lcard${isActive ? ' sel' : ''}" data-list-id="${list.id}" onclick="pickList('${list.id}')" role="button" tabindex="0" aria-label="${t('lists.select')} ${esc(list.name)}" aria-pressed="${isActive}">
    ...
    if (typeof Sortable !== 'undefined') {
      ...
      listsSortable = new Sortable(grid, {
        animation: 150,
        draggable: '.lcard',
        delay: 100,
        ...
        onEnd: function(evt) {
          const newOrder = [];
          document.querySelectorAll('#lgrid .lcard').forEach(card => {
            const listId = card.dataset.listId;
            if (listId) newOrder.push(listId);
          });
          const orderedLists = newOrder.map(id => lists.find(l => l.id === id)).filter(l => l);
          lists = orderedLists;
          saveData();
          renderLists();
        }
    ...
    if (listSearchActive && lastListQuery) {
      filterLists(lastListQuery);
    }
  }
}
...
function openSheet(sheetId) {
  ...
  if (openSheetId && openSheetId !== sheetId) {
    const oldSheet = document.getElementById(openSheetId);
    if (oldSheet) {
      oldSheet.classList.remove('on');
...
function forceCloseSheet(sheetId) {
  const sheet = document.getElementById(sheetId);
  const ovl = document.getElementById('ovl');
  if (sheet) {
    sheet.classList.remove('on');
...
function startBarcodeScan() {
  // Si le scan est déjà en cours, on ne fait rien
  if (html5QrCode) return;

  // Fonction interne pour initialiser le scanner une fois la bibliothèque chargée
  const initScanner = () => {
  ...
  if (typeof Html5Qrcode === 'undefined') {
    ...
    script.onerror = () => {
      showSnack('Erreur de chargement de la bibliothèque de scan');
    };
    document.head.appendChild(script);
  } else {
    // La bibliothèque est déjà présente, on initialise directement
    initScanner();
  }
}
...
/* =============================================================
  27. NOUVELLES FONCTIONS : SÉLECTEUR D'EMOJI, CHANGER DE LISTE, RECHERCHE
============================================================= */

// Ouvrir le sélecteur d'emoji (CORRIGÉ)
function openEmojiPicker(targetId) {
  // Sauvegarder le sheet actuel
  previousSheetId = openSheetId;
  // Fermer le sheet actuel (si ouvert)
  if (openSheetId) {
    closeSheet();
  }
  ...
  EMOJIS.forEach(emoji => {
    html += `<button class="emoji-item" aria-label="${emoji}">${emoji}</button>`;
  });
  grid.innerHTML = html;
  ...
  grid.querySelectorAll('.emoji-item').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Fermer le sélecteur
      closeSheet();
      // Rouvrir le sheet précédent
      if (previousSheetId) {
        openSheet(previousSheetId);
        previousSheetId = null;
      }
    });
  });

  openSheet('shEmojiPicker');
}

// Ouvrir le sélecteur de liste
function openChangeListSheet() {
  // Sauvegarder l'ordre original si ce n'est pas déjà fait
  if (originalListsOrder.length === 0) {
    originalListsOrder = lists.map(l => l.id);
  }

  const container = document.getElementById('changeListContainer');
  if (!container) return;
  ...
  lists.forEach(list => {
    const isActive = (list.id === activeId);
    html += `
      <div class="srow" style="cursor: pointer; ${isActive ? 'background:var(--sur2);' : ''}" data-list-id="${list.id}">
        <span style="font-size:1.5rem;">${list.emoji}</span>
        <span style="flex:1; margin-left:0.5rem;">${esc(list.name)}</span>
        ${isActive ? '<span style="color:var(--p);">✓</span>' : ''}
      </div>
    `;
  });
  container.innerHTML = html;
  ...
  document.getElementById('btnSortAlpha').onclick = () => {
    lists.sort((a, b) => a.name.localeCompare(b.name, cfg.lang || 'fr'));
    openChangeListSheet(); // rafraîchir l'affichage
  };
  document.getElementById('btnSortOriginal').onclick = () => {
    const ordered = originalListsOrder.map(id => lists.find(l => l.id === id)).filter(l => l);
    lists = ordered;
    openChangeListSheet();
  };

  openSheet('shChangeList');
}

// Fonction pour filtrer les listes
function filterLists(query) {
  const grid = document.getElementById('lgrid');
  if (!grid) return;
  const cards = grid.querySelectorAll('.lcard');
  cards.forEach(card => {
    const name = card.querySelector('.lname')?.textContent.toLowerCase() || '';
...
// Afficher/masquer la barre de recherche des listes
function toggleListSearch() {
  const container = document.getElementById('listSearchContainer');
  ...
  if (container.style.display === 'none') {
    ...
    filterLists(''); // réafficher toutes les listes
  }
}

/* =============================================================
  28. ATTACHEMENT DES ÉCOUTEURS
============================================================= */
function attachListeners() {
  ...
  getEl('fab-search')?.addEventListener('click', toggleListSearch);
  ...
  // Nouveaux boutons pour le sélecteur d'emoji
  getEl('btnPickListEmoji')?.addEventListener('click', () => openEmojiPicker('lEmoji'));
  getEl('btnPickCatEmoji')?.addEventListener('click', () => openEmojiPicker('catEmoji'));
  getEl('btnCancelEmoji')?.addEventListener('click', () => {
    closeSheet();
    if (previousSheetId) {
      openSheet(previousSheetId);
      previousSheetId = null;
  ...
  getEl('btnChangeList')?.addEventListener('click', openChangeListSheet);

  // Bouton Annuler du sheet Export
  getEl('btnCancelExport')?.addEventListener('click', closeSheet);

  // Sheet Changer de liste
  getEl('btnCancelChangeList')?.addEventListener('click', closeSheet);

  // Recherche de listes
  getEl('listSearchInput')?.addEventListener('input', (e) => {
    lastListQuery = e.target.value.toLowerCase().trim();
    filterLists(lastListQuery);
  });
...
(function setupQrZoom() {
  ...
  const MAX_SCALE = 0.9; // Réduit à 0.9 pour éviter le zoom excessif
...
// =============================================================
//  EXPOSITION GLOBALE DES FONCTIONS UTILISÉES DANS LES ATTRIBUTS ONCLICK
// =============================================================
...
window.forceCloseSheet = forceCloseSheet;
